...
class EasyLevalViewController: UIViewController {
    ...
    @IBOutlet weak var EasyLevalProgressBar: UIProgressView!
...
//    }
    ...
    func setUpProgrssBar() {
        ...
        DispatchQueue.global().async {
            for i in 0...300 {
                let prog = Float(i) / 300
                usleep(4000000)
                DispatchQueue.main.async { () -> Void in
                    self.EasyLevalProgressBar.setProgress(prog, animated: true)
                }
            }
        }
    }
